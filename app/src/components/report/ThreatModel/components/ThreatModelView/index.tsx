...
const ThreatModelView: FC<ThreatModelViewProps> = ({
  ...
  const getNextStepButtons = useCallback(() => {
    ...
    if (!hasContentDetails?.applicationInfo) {
      buttons.push(
        <Button key="appInfoBtn" onClick={props.onApplicationInfoView}>
    ...
    if (!hasContentDetails?.architecture) {
      buttons.push(
        <Button key="architectureViewBtn" onClick={props.onArchitectureView}>
          Add Architecture
        </Button>,
      );
    }
    ...
    if (!hasContentDetails?.assumptions) {
      buttons.push(
        <Button key="assumptionsViewBtn" onClick={props.onAssumptionListView}>
          Add Assumptions
        </Button>,
      );
    }
    ...
    if (!hasContentDetails?.controls) {
      buttons.push(
        <Button key="controlsViewBtn" onClick={props.onControlListView}>
    ...
    if (!hasContentDetails?.threats) {
      buttons.push(
        <Button key="mitigationsViewBtn" onClick={props.onMitigationListView}>
          Add Mitigations
        </Button>,
      );
    }
    const len = buttons.length;
    return buttons.flatMap((b, index) =>
      index === len - 1 ? (
        <Box>{b}</Box>
      ) : (
        [
          b,
          <Box fontWeight="bold" css={styles.text}>
            or
          </Box>,
        ]
      ),
          ...
          <Header
            actions={
              <SpaceBetween direction="horizontal" size="xs">
                ...
                <Button
                  key="onPrintButtonClickBtn"
                  variant="primary"
                  onClick={onPrintButtonClick || (() => window.print())}
                >
                  Print
                </Button>
              </SpaceBetween>
