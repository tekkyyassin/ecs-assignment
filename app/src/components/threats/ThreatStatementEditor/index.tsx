...
const ThreatStatementEditorInner: FC<{
  editingStatement: TemplateThreatStatement;
}> = ({ editingStatement }) => {
  const { applicationInfo } = useApplicationInfoContext();
  const { setEditingStatement, saveStatement, addStatement, onThreatListView } =
    useThreatsContext();
  ...
  const [editor, setEditor] = useState<ThreatFieldTypes | undefined>(
  ...
  const handleComplete = useCallback(() => {
    if (editingStatement) {
      ...
      if (editingStatement.numericId === -1) {
        ...
        linkedMitigationIds &&
          linkedMitigationIds.length > 0 &&
          addMitigationLinks(
            linkedMitigationIds.map((lm) => ({
              mitigationId: lm,
              linkedId: id,
            })),
          );
        linkedControlIds &&
          ...
          addControlLinks(
            linkedControlIds.map((lm) => ({
              controlId: lm,
              linkedId: id,
            })),
          );
      } else {
        const toAddlinkedAssumptionIds = linkedAssumptionIds.filter(
          (x) => !prevLinkedAssumptionIds.includes(x),
        );
        const toRemovelinkedAssumptionIds = prevLinkedAssumptionIds.filter(
          (x) => !linkedAssumptionIds.includes(x),
        );
        toAddlinkedAssumptionIds &&
          toAddlinkedAssumptionIds.length > 0 &&
          addAssumptionLinks(
        ...
        toRemovelinkedAssumptionIds &&
          toRemovelinkedAssumptionIds.length > 0 &&
          removeAssumptionLinks(
        ...
        const toAddlinkedMitigationIds = linkedMitigationIds.filter(
          (x) => !prevLinkedMitigationIds.includes(x),
        );
        const toRemovelinkedMitigationIds = prevLinkedMitigationIds.filter(
          (x) => !linkedMitigationIds.includes(x),
        );
        toAddlinkedMitigationIds &&
          toAddlinkedMitigationIds.length > 0 &&
          addMitigationLinks(
            toAddlinkedMitigationIds.map((lm) => ({
              mitigationId: lm,
              linkedId: id,
            })),
          );
        toRemovelinkedMitigationIds &&
          toRemovelinkedMitigationIds.length > 0 &&
          removeMitigationLinks(
            toRemovelinkedMitigationIds.map((lm) => ({
              mitigationId: lm,
              linkedId: id,
            })),
          );
        const toAddlinkedControlIds = linkedControlIds.filter(
          (x) => !prevLinkedControlIds.includes(x),
        );
        const toRemovelinkedControlIds = prevLinkedControlIds.filter(
          (x) => !linkedControlIds.includes(x),
        );
        toAddlinkedControlIds &&
          toAddlinkedControlIds.length > 0 &&
          addControlLinks(
            toAddlinkedControlIds.map((lm) => ({
              controlId: lm,
              linkedId: id,
            })),
          );
        toRemovelinkedControlIds &&
          toRemovelinkedControlIds.length > 0 &&
          removeControlLinks(
            toRemovelinkedControlIds.map((lm) => ({
              controlId: lm,
              linkedId: id,
            })),
          );
  ...
  const saveButtonText = useMemo(() => {
    ...
    return editingStatement?.numericId === -1
      ? `Add to workspace ${workspace}`
      : `Save to workspace ${workspace}`;
  }, [currentWorkspace, workspaceList, editingStatement]);

  const handleCustomTemplateConfirm = useCallback(
    (template: string) => {
            ...
            {composerMode === 'Full' && (
              <div css={styles.metadataContainer}>
                <ControlLookupComponent
                  ...
                  onRemoveControlLink={(id) =>
                    setLinkedControlIds((prev) => prev.filter((p) => p !== id))
                  }
                />
              </div>
            )}
...
const ThreatStatementEditor: FC = () => {
  ...
  return editingStatement ? (
    <ThreatStatementEditorInner editingStatement={editingStatement} />
  ) : null;
};
