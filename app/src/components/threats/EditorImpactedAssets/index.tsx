...
const EditorImpactedAssets: FC<EditorProps> = forwardRef<AutosuggestProps.Ref, EditorProps>(
  ({ statement, setStatement, fieldData }, _ref) => {
    ...
    const handleSelect: NonCancelableEventHandler<AutosuggestProps.SelectDetail> = useCallback(
      ({ detail }) => {
    ...
    const handleRemoveAsset: NonCancelableEventHandler<TokenGroupProps.DismissDetail> = useCallback(
      ({ detail: { itemIndex } }) => {
        setStatement(
          (prevStatement) =>
            prevStatement && {
    ...
    return (
      <EditorLayout title={fieldData.displayTitle} description={fieldData.description}>
        <Autosuggest
