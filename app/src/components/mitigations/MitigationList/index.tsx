...
const MitigationList: FC = () => {
  const { mitigationList, removeMitigation, saveMitigation } = useMitigationsContext();

  const { addMitigationLinks, mitigationLinkList, removeMitigationLinksByMitigationId } =
    useMitigationLinksContext();

  const { addAssumptionLinks, assumptionLinkList, removeAssumptionLinksByLinkedEntityId } =
    useAssumptionLinksContext();

  const { addControlLinks, controlLinkList, removeControlLinksByLinkedEntityId } =
    useControlLinksContext();
  ...
  const handleRemoveTagFromEntity = useCallback(
    (assumption: Mitigation, tag: string) => {
      const updated: Mitigation = {
        ...assumption,
  ...
  return (
    <div>
      <SpaceBetween direction="vertical" size="s">
        <Container header={<Header counter={`(${filteredList.length})`}>Mitigation List</Header>}>
          <SpaceBetween direction="vertical" size="s">
            <TextFilter
        ...
        <MitigationCreationCard onSave={handleSaveNew} />
      </SpaceBetween>
    </div>
  );
};
