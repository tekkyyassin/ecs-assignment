...
const LinkedEntityFilter: FC<LinkedEntityFilterProps> = ({
  ...
  return (
    <div css={styles.root}>
      ...
      <SegmentedControl
        selectedId={selected}
        onChange={({ detail }) => setSelected(detail.selectedId)}
        options={[
          {
            iconAlt: `Without linked ${entityDisplayName}`,
            id: WITHOUT_NO_LINKED_ENTITY,
            iconSvg: (
              <Tooltip tooltip={`Show ${callerName} without linked ${entityDisplayName}`}>
                <svg
                  css={svgStyles}
                  aria-hidden="true"
                  viewBox="0 0 24 24"
          ...
          },
          {
            ...
            iconSvg: (
              <Tooltip tooltip={`Show ${callerName} with linked ${entityDisplayName}`}>
