...
const useAssumptionLinks = (
  ...
  const handleRemoveAssumptionLinks = useCallback(
  ...
  const handlRemoveAssumptionLinksByAssumptionId = useCallback(
    async (assumptionId: string) => {
      setAssumptionLinkList((prevList) =>
        prevList.filter((x) => !(x.assumptionId === assumptionId)),
      );
    },
    [setAssumptionLinkList],
  );
  ...
  const handleAddAssumptionLinks = useCallback(
    (assumptionLinks: AssumptionLink[]) => {
      setAssumptionLinkList((prevList) => {
        const filteredLinks = assumptionLinks.filter(
          (al) => prevList.findIndex((pl) => isSameAssumptionLink(al, pl)) < 0,
        );
  ...
  const handleGetLinkedAssumptionLinks = useCallback(
